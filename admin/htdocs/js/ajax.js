/*
	[HuangYe88] (C) 2009-2010 HY88 Inc.
	$Id: script_ajax.js 2009/12/31 10:20:50Z Arli.Yao $
*/
...
function Ajax(recvType, waitId) {
	...
	aj.get1 = function(targetUrl,obj) {	
		...
		if(aj.XMLHttpRequest.readyState == 4 && aj.XMLHttpRequest.status == 200) {
			if(aj.recvType == 'HTML') {
				ajaxgetresult = aj.XMLHttpRequest.responseText;
			} else if(aj.recvType == 'XML') {
				try {
					ajaxgetresult = aj.XMLHttpRequest.responseXML.lastChild.firstChild.nodeValue;
				} catch(e) {
					ajaxgetresult = '';
				}
			}
		}
		return ajaxgetresult;
	}
...
function ajaxselect(url, selectid, waitid, dval,sval) {
	waitid = typeof waitid == 'undefined' || waitid === null ? showid : waitid;
	var x = new Ajax();
	x.setLoading();
	x.setWaitId(waitid);
	x.display = '';
	x.dval = typeof dval == 'undefined' || dval === null ? '----' : dval;
	x.sval = typeof sval == 'undefined' || sval === null ? 0 : sval;
		...
		if(!evaled) {
			...
			if(s==''){
				...
				if(x.dval=='no'){
					for(var i =0;i < len;i++){
						var nd = d[i].split(":")
						obj.options[i] = new Option(nd[1],nd[0]);
						if(nd[0]==x.sval){obj.options[i].selected = true;}
					}
				}else{
					obj.options[0] = new Option(x.dval,"0");
					for(var i =0;i < len;i++){
						var nd = d[i].split(":")
						obj.options[i+1] = new Option(nd[1],nd[0]);
						if(nd[0]==x.sval){obj.options[i].selected = true;}
					}
				}
			}
...
function popup(e, ctrlid, s, isbox, timeout, func) {
	...
	s = '<h1>提示信息</h1><a href="javascript:hideMenu();" class="float_del" title="关闭">关闭</a><div class="popupmenu_inner">' + s + '<div>';
	if(div) div.innerHTML = s;
	showMenu(ctrlid, 'click', offset, duration, timeout, 0, ctrlid, 1000, true);
	//function
	if(func) {
		setTimeout(func + '(\'' + ctrlid + '\')', 10);
	}
