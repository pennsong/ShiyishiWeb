...
<table cellspacing="0" cellpadding="0" width="100%" id="ftable">
  ...
  <tr class="tr3">
	...
	<td class="tal"><!--{if $item.resume_txt==1}--><a class="fourm-two" href="<!--{$baseurl}-->/admin/txtresume.do?uid=<!--{$item.id}-->">文字</a><!--{/if}--><!--{if $item.resume_vod==1}--> | <a class="fourm-two" href="<!--{$baseurl}-->/admin/videoresume.do?uid=<!--{$item.id}-->">视频[<!--{$item.auth}-->]</a><!--{/if}--></td>
