...
<!--{foreach from=$rows item=item}-->
	<tr class="tr3" id="tr_<!--{$item.id}-->">
		<td class="tal">
			...
			<span class="fourm_name"><!--{$item.prename|default:$item.thename}--></span><!--{if $item.parent_id==0}--><span id="s_<!--{$item.id}-->"><a onclick="showSubs(<!--{$item.id}-->);" style="cursor:pointer;color:red;" title="查看子行业">+</a></span><!--{/if}-->
		</td>
		<td><!--{$item.enname}--></td>
...
function showSubs(pid){
	$.ajax({
		type:"GET",
		url:"<!--{$baseurl}-->/admin/ajax/getsubdtype.do",
		dataType:"html",
		data:"pid="+pid,
		...
		success:function(msg){
			if(!msg){
				...
				$("#s_"+pid).html('<a onclick="hideSubs('+pid+');" style="cursor:pointer;color:red;" title="隐藏子行业">-</a></span>');
				$("#tr_"+pid).after(msg);
			}
		}
	});
...
function hideSubs(pid){
	for(var i=0;i<100;i++){
		if(document.getElementById('subtr_'+pid+'_'+i)){
			...
			$("#s_"+pid).html('<a onclick="showSubs('+pid+');" style="cursor:pointer;color:red;" title="显示子行业">+</a></span>');
		}else{
			break;
		}
	}
