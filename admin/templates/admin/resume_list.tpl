...
<div style="float:right;margin:0px 0 0 0;#margin:0px 0 0 0;_margin:-30px 0 0 0;">
	...
	<!--{foreach from=$job_category item=item}-->
		<option value="<!--{$item.id}-->" <!--{if $s.jid == $item.id}-->selected="selected"<!--{/if}-->><!--{$item.name}--></option>
	<!--{/foreach}-->
	</select>	
	公司:
	<select name="cid">	
	...
	<!--{foreach from=$e_user item=item}-->
		<option value="<!--{$item.id}-->" <!--{ if $s.cid == $item.id}-->selected="selected"<!--{/if}-->><!--{$item.username}--></option>
	<!--{/foreach}-->
	</select>	
	时间:
