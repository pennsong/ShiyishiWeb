                		...
                		<div id="hrzhiweimain">
							...
							<input  name="info[id]" type="hidden" value="<!--{$info.id}-->" />
                                ...
                                <li><span class="glogintext">学历要求：</span>
									<select name="info[degree]" id="degree" alt="学历要求:空"  onchange="$('#showResult_degree').html('');if(this.value=='8'){ $('#toall').html('');}else{$('#toall').html('及以上');}">
									<option value="">请选择</option>
										<!--{foreach from=$degrees key=key item=item}-->
										<option value="<!--{$key}-->"<!--{if $info.degree==$key}--> selected<!--{/if}-->><!--{$item}--></option>
										<!--{/foreach}-->
									</select> <span id="toall"></span>
...
function submitCheck(){
	var str = '<img src="/images/no.jpg"><font color="red"> 该项目未选择</font>';
	//alert($('#live').val());
	//return false;
	if($('#live').val() == 0){
	...
	if( document.getElementById('title').value=="" ){
		$('#showResult_title').html('<img src="/images/no.jpg"><font color="red">标题不可为空</font>');
		return false;
	}
//	if($('#scid').val() == 0){
