...
function endIntView(){
	if(confirm('您确定要结束今天的面试，点否则面试下一位。')){
		window.top.location.href='<!--{$baseurl}-->/enterp/interview/over.html?id=<!--{$roomid|default:0}-->';
	}else{
		interNext();
	}
...
function loadswf(){
	...
	if(isIE){
		...
		musicStr+='<param name="movie" value="/images/interview0426.swf?id=<!--{$user|default:"user"}-->&role=<!--{$roleid|default:1}-->&userid=<!--{$uid|default:0}-->&roomid=<!--{$roomid|default:0}-->&apurl=rtmp://www.channelwin.info:8008/<!--{$room|default:"room1"}-->" />';
		musicStr+='<param name="allowScriptAccess" value="always" />';
		musicStr+='<param name="quality" value="high" />';
		musicStr+='<embed name="interviewonline" src="/images/interview0426.swf?id=<!--{$user|default:"user"}-->&role=<!--{$roleid|default:1}-->&userid=<!--{$uid|default:0}-->&roomid=<!--{$roomid|default:0}-->&apurl=rtmp://www.channelwin.info:8008/<!--{$room|default:"room1"}-->" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" allowScriptAccess="always" width="736" height="672"></embed>';
		musicStr+='</object>';
		...
		flashvars.apurl = 'rtmp://www.channelwin.info:8008/room2" />';
		var params = {};
		params.quality = "high";
		...
		swfobject.embedSWF("/images/interview0426.swf", "msright",  "736", "672", swfVersionStr, xiSwfUrlStr,  flashvars, params, attributes);
	}
}
