...
class mtsms_Model extends Model{
	...
	function httpsend($mobiles,$smscontent){
		echo $content = file_get_contents('http://3tong.cn:8082/ema_new/http/SendSms?Account='.$this->mtsms['account'].'&Password='.$this->mtsms['password'].'&SubCode=&Phone='.$mobiles.'&Content='.$smscontent);
		if($content>0){
			$this->logstr("发送".$smscontent."   至".$mobiles."   成功   ");
			return 'success';
		}else{
